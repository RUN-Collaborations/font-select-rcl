...
function Component(){
  ...
  // Do we have font features for the selected font?
  const fontIfListed = useMemo(() => graphiteEnabledFeatures.filter((name) => name.name === selectedFont).map(({ name }) => name),[selectedFont]);
	
  // Show Graphite enabled features?
  ...
  const fontSettingsJsx = useMemo(() => graphiteEnabledFeatures.filter((name) => name.name === selectedFont).map((font, fontIndex) => (
    <div key={fontIndex}>
  ...
  const labelJsxText = useMemo(() => graphiteEnabledFeatures.filter((name) => name.name === selectedFont).map((font, fontIndex) => (
    <div key={fontIndex}>
      {font.categories.map((categories, categoriesIndex) => {
        return (<div key={categoriesIndex}>
          {font.categories[categoriesIndex].category.map((category, categoryIndex) => {
          ...
          >
            ...
            >
              <p style={{ fontSize: "0.9em", margin: "0px" }}>
                ...
                (for copy-paste)
                <br />
                {graphiteEnabledSettings && labelDir === 'rtl' && '...or copy label-text from ' + selectedFont + ' selected font features!'}
              </p>
            </div>
            <div
              style={{
